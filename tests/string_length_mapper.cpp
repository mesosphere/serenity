...
#include "string_length_mapper.hpp"


namespace mesos {
namespace serenity {

StringLengthMapper::~StringLengthMapper() {}

Try<Nothing> StringLengthMapper::consume(std::string value)
{
  produce(value.length()); // emit the string length
  return Nothing();
}
