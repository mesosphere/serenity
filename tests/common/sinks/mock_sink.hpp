...
void expect_eq_double(double_t lvalue, double_t threshold, double_t rvalue) {
  EXPECT_GT((lvalue + threshold), rvalue);
  EXPECT_LT((lvalue - threshold), rvalue);
}
...
class MockSink : public Consumer<T> {
 public:
  ...
  void expectIpc(
    ...
    expect_eq_double(
        stats.net_tcp_active_connections(), threshold, value);
  }
  ...
  void expectCpuUsage(
    ...
    expect_eq_double(
        stats.net_tcp_time_wait_connections(), threshold, value);
  }
