...
#include "filters/ema_filter.hpp"
...
TEST(EMATest, SmoothingConstSample) {
  ...
  for (; loadGen.end() ; loadGen++) {
    ...
    expect_eq_double((*loadGen).clearValue(), THRESHOLD, result);
  }
}
...
TEST(EMATest, IpcEMATest) {
  ...
  expect_eq_double(RESULT_EXECUTOR1, THRESHOLD, ipc);

  ipc = usage.get().executors(1).statistics().net_tcp_active_connections();
  expect_eq_double(RESULT_EXECUTOR2, THRESHOLD, ipc);
}
...
TEST(EMATest, CpuUsageEMATest) {
  ...
  expect_eq_double(RESULT_EXECUTOR1, THRESHOLD, cpuUsage);

  cpuUsage =
    usage.get().executors(1).statistics().net_tcp_time_wait_connections();
  expect_eq_double(RESULT_EXECUTOR2, THRESHOLD, cpuUsage);
}
